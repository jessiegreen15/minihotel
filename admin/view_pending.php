				...
				<div class="container">
					<form method="POST" enctype="multipart/form-data" action="update_query_pending.php?transaction_id=<?php echo $fetch['transaction_id'] ?>">
						<div class="user-details">
							...
							<div class="input-box">
								<span class="details">Downpayment</span>
								<input type="text" placeholder="" name="downpayment" id="down" value="<?php echo $fetch['downpayment'] / 100 *  $fetch['price'] * $fetch['days'] ?>" disabled />

							</div>
							<div class="input-box">
								<span class="details">Enter amount</span>
								<input type="number" placeholder="Enter amount" id="changes" name="changes" value="" required />
							</div>
						...
						<div class="button">
							...
							<button onclick="confirmationdisapprove(this); return false;" name="disapprove" class="btn btn-warning"> Disapprove</button>
						</div>
				</div>
...
<script>
	...
	function confirmationdisapprove(anchor) {
		var conf = confirm("Are you sure you want to disapprove this reservation?");
		if (conf) {
			window.location = anchor.attr("");
		}
	}
