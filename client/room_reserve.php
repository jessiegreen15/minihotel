	...
	<div class="container-fluid" style="max-width: 1500px; position: relative; margin-top:5%;  ">
		<div class="panel panel-default">
			<div class="panel-body">
				<div class="alert alert-info">Room Reserved
				</div>
				<?php
				include('profile.php');
				?>
				...
				<table id="table" class="table table-bordered">
					...
					<tbody>
						<?php
						$query = $conn->query("SELECT * FROM `room_transaction` NATURAL JOIN `guest`  NATURAL JOIN `room` NATURAL JOIN room_type WHERE `status` = 'pending' && `username` = '$fetch[username]' ORDER BY transaction_id");
						...
						?>
							<tr>
								...
								<td><?php echo $fetch['days'] * $fetch['price'] * '0.3' ?></td>
								<td><?php echo $fetch['status'] ?></td>
								<td>
									<center><a class="btn btn-success" onclick="confirmationcancel(this); return false;" name="cancelled" href="cancel_pending.php?transaction_id=<?php echo $fetch['transaction_id'] ?>"><i class="glyphicon glyphicon-check"></i> Cancel </a>
								</td>
...
<script type="text/javascript">
	function confirmationcancel(anchor) {
		var conf = confirm("Are you sure you want cancel this reservation?");
		if (conf) {
