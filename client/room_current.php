...
<?php
	...
	<div class = "container-fluid" style="max-width: 1500px; position: relative; margin-top:5%;  ">
		<div class = "panel panel-default">
			...
			<div class = "alert alert-info">Check In Status
            </div>
				<?php
				include('profile.php');
				?>
				...
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `room_transaction` NATURAL JOIN `guest`  NATURAL JOIN `room` NATURAL JOIN room_type WHERE `status` = 'Check In' && `username` = '$fetch[username]'");
