...
<?php
	...
	<div class = "container-fluid" style="max-width: 1500px; position: relative; margin-top:5%;  ">
		<div class = "panel panel-default">
			...
			<div class = "alert alert-info">Room Status / Approve
            </div>
				<?php
				include('profile.php');
				?>
				...
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `room_transaction` NATURAL JOIN `guest`  NATURAL JOIN `room` NATURAL JOIN room_type WHERE `status` = 'approveD' && `username` = '$fetch[username]'");
