...
<?php
	...
	<div class = "container-fluid" style="max-width: 1500px; position: relative; margin-top:5%; ">
		<div class = "panel panel-default">
			...
			<div class = "alert alert-info">Event Reserved
            </div>
				<?php
...
<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `event_transaction` NATURAL JOIN `guest` NATURAL JOIN `event_type` WHERE `status` = 'Pending' && `firstname` = '$fetch[firstname]' && `middlename` = '$fetch[middlename]' && `lastname` = '$fetch[lastname]'");
						...
						<tr>
							...
							<td><center><a class = "btn btn-success" name="cancelled" href = "event_cancel_pending.php?transaction_id=<?php echo $fetch['transaction_id']?>"><i class = "glyphicon glyphicon-check"></i> Cancel </a></td>
						</tr>
						<?php
							}
						?>
